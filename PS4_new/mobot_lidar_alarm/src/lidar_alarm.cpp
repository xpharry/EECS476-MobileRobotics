...
int ping_index_min_ = 0;
int ping_index_max_ = 0;
...
double scan_width_ = 0.5;
double dangerPercentage = 0.3;
...
void laserCallback(const sensor_msgs::LaserScan& laser_scan) {
    if (ping_index_<0)  {
        ...
        double phi = atan2(scan_width_, 2*MIN_SAFE_DISTANCE);
        ping_index_min_ = ping_index_ - phi/angle_increment_;
        ping_index_max_ = ping_index_ + phi/angle_increment_;
        ROS_INFO("LIDAR setup: ping_index = %d",ping_index_);
        ROS_INFO("LIDAR setup: ping_index_min = %d",ping_index_min_);
        ROS_INFO("LIDAR setup: ping_index_max = %d",ping_index_max_);
    }
    ...
    for (int i = ping_index_min_; i < ping_index_max_; ++i) {
      ...
      if (laser_scan.ranges[i] < MIN_SAFE_DISTANCE) {
        numDanger++;
        averageDist += laser_scan.ranges[i];
      }
    }

    if (numDanger > (ping_index_max_ - ping_index_min_)*dangerPercentage) {
