...
/* below are methods added in ps9 by Peng */

double PclUtils::compareColor(Eigen::Vector3i color1, Eigen::Vector3i color2) {
    double diff_r = abs(color1[0] - color2[0]) / 255.0;
    double diff_g = abs(color1[1] - color2[1]) / 255.0;
    double diff_b = abs(color1[2] - color2[2]) / 255.0;
    return sqrt(diff_r*diff_r + diff_g*diff_g + diff_b*diff_b);
}

void PclUtils::graspPointsByColor(PointCloud<pcl::PointXYZRGB>::Ptr inputCloud, PointCloud<pcl::PointXYZRGB>::Ptr outputCloud, Eigen::Vector3i color) {
    int npts = inputCloud->points.size(); //how many points to extract?
    outputCloud->header = inputCloud->header;
    outputCloud->is_dense = inputCloud->is_dense;
...
bool PclUtils::isWithinLimit(Eigen::Vector3f toCompute, Eigen::Vector3f lower_limit, Eigen::Vector3f upper_limit) {
    for(int i = 0; i < 3; i++) {
        if(toCompute[i]<lower_limit[i] || toCompute[i]>upper_limit[i]) {
            return false;
        }
    }
    return true;
...
void PclUtils::graspPointsBySpace(PointCloud<pcl::PointXYZRGB>::Ptr inputCloud, PointCloud<pcl::PointXYZRGB>::Ptr outputCloud) {
    ...
    for (int i = 0; i < npts; ++i) {
        pcl::PointXYZRGB point = inputCloud->points[i];
        if(isWithinLimit(point.getVector3fMap(), lower_limit, upper_limit)) {
            // ROS_INFO("within limit ... add in ...");
            outputCloud->points.push_back(point);
        }
    }
...
void PclUtils::graspPointsBySpace(PointCloud<pcl::PointXYZRGB>::Ptr inputCloud, PointCloud<pcl::PointXYZRGB>::Ptr outputCloud,
            Eigen::Vector3f lower_limit, Eigen::Vector3f upper_limit) {
...
void PclUtils::detectStool(PointCloud<pcl::PointXYZRGB>::Ptr inputCloud, PointCloud<pcl::PointXYZRGB>::Ptr outputCloud) {
    ...
    // for (int i = 0; i < npts; ++i) {
    //     pcl::PointXYZRGB point = inputCloud->points[i];
    //     if((compareColor(color, point.getRGBVector3i()) < 0.1) && isWithinLimit(point.getVector3fMap(), lower_limit, upper_limit)) {        
    //         ROS_INFO("stool color and within limit ... add in ...");
    //         outputCloud->points.push_back(point);
    //     }
    // }
...
void PclUtils::downgradeCloud(PointCloud<pcl::PointXYZRGB>::Ptr inputCloud, PointCloud<pcl::PointXYZ>::Ptr outputCloud) {
    ...
    outputCloud->points.resize(npts);
    for (int i = 0; i < npts; ++i) {
        if(!isNanVector3f(inputCloud->points[i].getVector3fMap())) {
            outputCloud->points[i].getVector3fMap() = inputCloud->points[i].getVector3fMap();
            // cout<<"*** input: "<<inputCloud->points[i].getVector3fMap().transpose()<<endl;
            // cout<<"*** output: "<<outputCloud->points[i].getVector3fMap().transpose()<<endl;
        }
    }
}

bool PclUtils::isNan(double f) {
    if(f != f) return true;
    return false;
}

bool PclUtils::isNanVector3f(Eigen::Vector3f f) {
    if(isNan(f[0]) || isNan(f[1]) || isNan(f[2])) return true;
    return false;
}